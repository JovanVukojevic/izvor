...
CREATE FUNCTION spec.create_course(p_title text, p_description text, p_category_id uuid) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_id UUID;
...
CREATE FUNCTION spec.update_course(p_id uuid, p_title text, p_description text, p_category_id uuid) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_cur_title  TEXT;
