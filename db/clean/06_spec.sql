...
CREATE FUNCTION spec.activate_user(p_user_id uuid) RETURNS SETOF api."user"
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION spec.create_user(p_email text, p_password_hash text, p_role text DEFAULT 'learner'::text) RETURNS SETOF api."user"
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_id      UUID;
...
CREATE FUNCTION spec.deactivate_user(p_user_id uuid) RETURNS SETOF api."user"
    LANGUAGE plpgsql
    AS $$
BEGIN
    PERFORM spec.assert_role('admin');
...
CREATE FUNCTION spec.create_category(p_name text, p_description text) RETURNS SETOF api.category
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_id UUID;
...
CREATE FUNCTION spec.update_category(p_id uuid, p_name text, p_description text) RETURNS SETOF api.category
    LANGUAGE plpgsql
    AS $$
BEGIN
    PERFORM spec.assert_role('admin');
...
CREATE FUNCTION spec.create_lesson(p_course_id uuid, p_title text, p_content text) RETURNS SETOF api.lesson
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_id       UUID;
...
CREATE FUNCTION spec.reorder_lesson(p_lesson_id uuid, p_new_position integer) RETURNS SETOF api.lesson
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_course_id UUID;
...
CREATE FUNCTION spec.update_lesson(p_lesson_id uuid, p_title text, p_content text) RETURNS SETOF api.lesson
    LANGUAGE plpgsql
    AS $$
DECLARE
    v_course_id UUID;
