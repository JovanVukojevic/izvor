...
-- -----------------------------
-- Grants — izvor_admin (full access)
-- -----------------------------
...
-- -----------------------------
-- Grants — izvor_app (least-privilege runtime)
-- -----------------------------
...
-- Explicitly NO grant on impl, spec, system_impl, system_spec.

-- Default privileges: future objects izvor_admin creates in
-- api / system_api / app auto-grant to izvor_app.
...
-- -----------------------------
-- Session-context helper functions (fail-closed)
-- -----------------------------
-- Read session variables set by the .NET backend at the start
-- of each transaction (via `SET LOCAL app.current_tenant = ...`).
-- If unset or empty, raise — never silently return NULL.
...
-- Ownership
-- -----------------------------
-- Transfer ownership of everything created above (by the postgres
-- superuser) to izvor_admin, so that izvor_admin's default-privilege
-- rules apply to its future-created objects in these schemas.
