...
public sealed class LessonsEndpointTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        var lessons = await AnaClient().GetAsync($"/api/courses/{_courseId}/lessons");
        var lessonList = (await lessons.Content.ReadFromJsonAsync<List<LessonResponse>>())!;
        _firstLessonId = lessonList[0].Id;
    }
    ...
    {
        // The fixture course already has its first lesson at position 1;
        // a newly-added lesson lands at position 2.
        var ana = AnaClient();
    ...
    [Fact] // L5
    public async Task List_returns_fixture_first_lesson_at_position_1()
    {
        // Post-027 every course ships with its first lesson inline, so the
        // "empty list" state is no longer reachable from the public surface.
        // The fixture lesson is the only one present until other tests add more.
        ...
        list[0].Id.Should().Be(_firstLessonId);
    ...
    public async Task List_returns_lessons_in_position_order()
    {
        ...
        list[0].Id.Should().Be(_firstLessonId);
        list[0].Position.Should().Be(1);
        list[1].Title.Should().Be("A");
        list[1].Position.Should().Be(2);
        list[2].Title.Should().Be("B");
        list[2].Position.Should().Be(3);
        list[3].Title.Should().Be("C");
        list[3].Position.Should().Be(4);
    }
    ...
    [Fact] // L15
    public async Task Delete_lesson_with_completions_returns_409_lesson_has_completions()
    {
        ...
        body.Message.Should().Be("lesson_has_completions");
    }

    [Fact] // L16
    public async Task Reorder_moves_lesson_to_new_position()
    {
        // Fixture provides position 1; tests-added A/B/C land at 2/3/4.
        // Reordering C to position 1 shifts the fixture, A, and B down by one.
        ...
        list[1].Id.Should().Be(_firstLessonId);
        list[2].Id.Should().Be(a.Id);
        list[3].Id.Should().Be(b.Id);
    }
