...
public sealed class LessonProgressEndpointTests : IAsyncLifetime
{
    ...
    public async Task Get_progress_by_enrollment_for_self_succeeds()
    {
        ...
        var response = await PeraClient().GetAsync($"/api/enrollments/{_peraP1EnrollmentId}/progress");
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var list = (await response.Content.ReadFromJsonAsync<List<LessonProgressResponse>>())!;
    ...
    public async Task Get_progress_by_enrollment_for_course_author_succeeds()
    {
        ...
        var response = await AnaClient().GetAsync($"/api/enrollments/{_peraP1EnrollmentId}/progress");
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var list = await response.Content.ReadFromJsonAsync<List<LessonProgressResponse>>();
        list!.Should().HaveCount(1);
    }
    ...
    public async Task Get_progress_by_enrollment_for_admin_succeeds()
    {
        ...
        var response = await AdminClient().GetAsync($"/api/enrollments/{_peraP1EnrollmentId}/progress");
        response.StatusCode.Should().Be(HttpStatusCode.OK);
    }
    ...
    public async Task Get_progress_by_enrollment_for_unrelated_learner_returns_403()
    {
        ...
        var response = await IvanaClient().GetAsync($"/api/enrollments/{_peraP1EnrollmentId}/progress");
